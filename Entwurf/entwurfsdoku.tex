...
Der Grobentwurf (dargestellt in Abbildung \ref{fig:Paketdiagramm_Grobentwurf}) vereint mehrere Architekturmuster. Zum einen kann die Struktur als Repository-Architektur angesehen werden, da es verschiedene Ansichten gibt, die alle auf die gleiche Datenbasis zugreifen, das Repository. Die Schnittstelle zwischen den Clients und der Datenbasis bildet das Paket Repository.
Betrachtet man einen einzelnen Client, so kann dessen Struktur wiederum als 3-Schichtenarchitektur gesehen werden, in der das Repository-Paket der Datenhaltungsschicht entspricht, die Models der Logikschicht und Views und Presenter der Präsentationsschicht. Die oberen beiden Schichten wiederum, in denen sich Models, Views und Presenter befinden, wurden nach dem Model-View-Presenter-Entwurfsmuster gestaltet.
